...
class timeline {
    ...
    /**
     * leave mark on timeline
     *
     * @param label {string}
     * @param ts {date_time}
     * @param detail {any}
     */
    mark(label, ts = null, detail = null) {
        this.#labels.push({label, start: new date_time(ts), detail});
    }

    toJSON() {
        const times = this.#labels.map(x => x.start).sort();
    }
}
